...
//     i2c_
//     i2c_status_t status = i2c_init(address, timeout);
...
i2c_status_t tiny85_i2c_tx(const uint8_t * data, uint8_t length) {
    ...
    return i2c_transmit(TINY_ADDR_WR, data, length, I2C_DEF_TIMEOUT);
}
...
bool tinyrgb_task(void) {
    ...
    uint8_t ans = i2c_transmit(TINY_ADDR_WR, tinyrgb_cmd_buf, tinyrgb_cmd_len, I2C_DEF_TIMEOUT);
